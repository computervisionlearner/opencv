...
def add_hat(img,hat_img):
  ...
  if len(dets)>0:  
    for d in dets:
      ...
      #  根据人脸大小调整帽子大小
      factor = 1.5
      resized_hat_h = int(round(rgb_hat.shape[0]*w/rgb_hat.shape[0]*factor))
...
def batch_process():
  ...
  for path in img_path:
    ...
    # 展示效果
    cv2.namedWindow('{}'.format(name),0)
    cv2.imshow("{}".format(name), output)
